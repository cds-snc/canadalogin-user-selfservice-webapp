...
import {
    ...
    GcdsText,
    GcdsNotice,
    GcdsButton, GcdsGrid, GcdsLink, GcdsStepper
...
export default function ConfirmContactPhoneNumberUpdate({ step, totalSteps, onNext, onCancel, onChangePhoneForm, phoneFormData, userProfile }) {
    const { language } = useParams();
    const { state, dispatch } = useUser();
    const { clearEditProfile, updateProfileSuccess } = userProfileDispatch(dispatch);
    const pageContentJson = getPageContent(language, PAGES.confirmContactPhoneNumberUpdate);
        ...
        <GcdsContainer>
            <GcdsGrid columns="1" gap="300">
                ...
                <GcdsText marginBottom="0">
                    {pageContentJson["2"]} <strong>{phoneFormData.formattedPhoneNumber}</strong>.
                </GcdsText>
                <GcdsText >{pageContentJson["4"]}
                    <ul>
                ...
                <GcdsNotice type="info" noticeTitleTag="h2" noticeTitle=' '>
                    <GcdsText>
                        ...
                        <GcdsText>
                            {pageContentJson["8"]} <GcdsLink href="https://accounts.gc.ca/directory">{pageContentJson['9']}</GcdsLink>
                        </GcdsText>
                    </GcdsText>
                </GcdsNotice>
