...
vi.mock("react-i18next", () => ({
  useTranslation: () => ({
    t: (key: string) =>
      (
        {
          "successNotice.title": "Your information was successfully saved in CanadaLogin",
...
vi.mock("../../../../utils/constants", () => ({
  ...
  EXTERNAL_NAVIGATION_LINKS: {
    gcAccountDirectory: "https://www.canada.ca/en/government/sign-in-online-account.html",
  },
}));

vi.mock("@gcds-core/components-react", () => ({
  GcdsButton: ({ children, buttonRole }: React.PropsWithChildren<{ buttonRole?: string }>) => (
    <button data-button-role={buttonRole}>{children}</button>
  ),
  GcdsContainer: ({ children }: React.PropsWithChildren) => <div>{children}</div>,
  GcdsGrid: ({ children }: React.PropsWithChildren) => <div>{children}</div>,
  GcdsHeading: ({ children, tag }: React.PropsWithChildren<{ tag: "h1" | "h2" }>) => {
    const Tag = tag;
  ...
  GcdsNotice: ({ children, noticeRole, noticeTitle }: React.PropsWithChildren<{ noticeRole: string; noticeTitle: string }>) => (
...
describe("ConnectedServicesPage", () => {
  it("renders the success notice, service sessions, and actions in development", () => {
    ...
    expect(screen.getByRole("button", { name: "Sign out everywhere" })).toHaveAttribute(
      "data-button-role",
      "danger",
    );
    expect(screen.getByRole("button", { name: "I'll do this later" })).toHaveAttribute(
      "data-button-role",
      "secondary",
    );
    expect(screen.getByRole("link", { name: "Government of Canada account directory" })).toHaveAttribute(
      "href",
