import App from '../App';
import {cleanup, render, screen, queryByAttribute, fireEvent} from '@testing-library/react';
...
describe('Routing Test', () => {
    ...
    test("Check home route with no language defined and the browser set to French", () => {
        const dom =render(
            <MemoryRouter initialEntries={['/']}>
                <App />
            </MemoryRouter>,
    ...
    function checkPasswordPageContents(language, pageContentJson) {
        ...
        Object.keys(pageContentJson).forEach(key => {
            if (key==='1')
                expect(screen.queryByLabelText("gcds-notice", {selector: 'gcds-notice'})).toHaveAttribute('notice-title', pageContentJson['1']);
            else if(key==='7')
                expect(screen.queryByLabelText("gcds-title", {selector: 'gcds-title'})).toHaveAttribute('details-title',pageContentJson['7']);
            else if(key==='11')
                expect(screen.queryByLabelText("gcds-checkbox", {selector: 'gcds-checkbox'})).toHaveAttribute("label",pageContentJson['11']);
            else if(key==='18')
                expect(screen.queryByLabelText("gcds-link", {selector: 'gcds-link'})).toHaveAttribute("href",pageContentJson['18']);
            else
