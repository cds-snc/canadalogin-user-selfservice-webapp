...
export default function ManageFIDO2() {
  ...
  const handleAddFIDO2 = async () => {
    if (!webAuthnSupported) {
      setError(errorPageContent["error_webauthn_not_supported"]);
      return;
    }

    if (!newDeviceName || newDeviceName.trim() === "") {
      setError(errorPageContent["error_passkey_name_required"]);
      return;
    }
    ...
    try {
      ...
      if (!attestationResponse?.success || !attestationResponse?.data) {
        setError(errorPageContent["error_failed_to_get_attestation_options"]);
        return;
      }
      ...
      if (response && response.success) {
        setSuccess("Security key successfully added");
        setNewDeviceName("");
      } else {
      ...
      if (err instanceof DOMException && err.name === "InvalidStateError") {
        setError(errorPageContent["error_duplicate_passkey"]);
      } else {
        setError(err?.data?.message || "error_failed_to_register_credential");
      }
    } finally {
