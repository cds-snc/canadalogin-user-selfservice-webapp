...
export default function Breadcrumbs() {
    ...
    const onIndexRoute = matches.length > 1 && matches[matches.length - 1].pathname.replace(/\/+$/, '') === matches[matches.length - 2].pathname.replace(/\/+$/, '');

    const routeCrumbs = matches.filter((match) => Boolean(match.handle?.breadcrumbId))
        .map((match) => ({
            name: getPageContent(language, match.handle.id)[match.handle.breadcrumbId],
            url: match.pathname
        })).slice(0, onIndexRoute ? -1 : undefined)

    const crumbsToRender = [rp, ...routeCrumbs].filter(crumb => Boolean(crumb));

    return <GcdsBreadcrumbs slot="breadcrumb" hideCanadaLink>
        {crumbsToRender.map((crumb, index) => (
            <GcdsBreadcrumbsItem key={index} href={crumb.url}>{crumb.name}</GcdsBreadcrumbsItem>
        ))}
    </GcdsBreadcrumbs>
}