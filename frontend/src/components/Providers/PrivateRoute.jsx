...
function PrivateRoute ({route, children}){
    ...
    if((route==="signUpVerifyTwoStep" && !state.userData.stepVerificationSent && !state.userData.emailValidated &&
            !isEmailValid(state.userData.email) && !state.userData.passwordSubmitted) ||
        (route==="signUpVerification" && !state.userData.emailValidated && !isEmailValid(state.userData.email) &&
