...
interface NoticeFactoryProps {
  noticeType: "mfaDeleted" | "mfaAdded" | "passkeyAdded" | "passkeyDeleted" | "passkeyRenamed";
  phoneNumber?: string;
  otpType?: string;
  passkeyName?: string;
}
...
export default function NoticeFactory({
   ...
   */
  const NoticeComponents: Record<string, (props: { phoneNumber?: string; otpType?: string; passkeyName?: string }) => React.ReactNode> = {
    ...
    mfaAdded: ({ phoneNumber, otpType }) => (
      <GcdsText>
        <GcdsNotice type="success" noticeTitleTag="h2" noticeTitle={" "}>
          <GcdsText>
            {(successBannerJson["3"] as string)} {otpType} {(successBannerJson["4"] as string)}{" "}
