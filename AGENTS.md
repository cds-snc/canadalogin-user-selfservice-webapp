...
- **Services/Utils**: `camelCase.jsx` or `.js` (e.g., `authService.tsx`, `apiErrorHandler.js`)
...
- No path aliases — use relative paths (`../../utils/constants.ts`).
...
- Access content via `getPageContent(language, PAGES.pageName)` from `utils/functions.ts`.
