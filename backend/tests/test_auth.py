...
class FakeVerifyClient:
    ...
    def __init__(self, transport: MockTransport):
        ...
        # Add client_kwargs to support code that sets PKCE and scope
        self.client_kwargs = {
...
def app(monkeypatch, mock_token_transport):
    ...
    async def reauth(request: Request, returnToPage: str = "/", acr_values: str = None):
        return await auth_module.reauthenticate_user(request, returnToPage=returnToPage, acr_values=acr_values)
...
async def test_reauthenticate_user_with_acr_values_for_stepup(app, client):
    resp = await client.get(
        "/reauth", params={"returnToPage": "/security", "acr_values": "loa3_stepup"}, follow_redirects=False
    )
    assert resp.status_code in (302, 307)
    assert resp.headers["location"].startswith(
        "https://idp.example/authorize?redirect_uri="
    ...
    assert app.state.oauth_verify.last_authorize_redirect_kwargs.get("acr_values") == "loa3_stepup"
