...
from app.users.services.login import requestCloudDirectoryId, getCloudDirectoryId, signin_with_password
...
async def test_request_cloud_directory_id_success():
    ...
    mock_response.json.return_value = {"password":[{"name":"Cloud Directory",
        "location":"https://cds-gcsignin-dev.verify.ibm.com/v1.0/authnmethods/password/d67ed8...-a376-2b357a9ef0e4",
        "id":"d67ed8ae-5971-423b-a376-2b357a9ef0e4","type":"ibmldap"}],"total":1,"count":200,"limit":200,"page":1}
...
async def test_request_cloud_directory_id_raises_http_exception():
    ...
    with (
        patch("app.users.services.login.get_admin_token", new_callable=AsyncMock) as mock_token,
        patch("app.users.services.login.get_auth_request_headers") as mock_headers,
        patch("app.users.services.login.get_settings") as mock_settings,
        patch("app.users.services.login.AsyncClient") as mock_client_class,
    ):
...
async def test_get_cloud_directory_id_failure():
    ...
    with (
        patch("app.users.services.login.requestCloudDirectoryId", return_value=mock_response) as dispatcher,
        patch("app.users.services.login.generate_error_response") as generate_error_response
    ):
...
async def test_signin_with_password_successfully():
    ...
    user_login_request_data = UserLoginRequestData(
        ...
        password = "frpY3wU*zm%+BL15"
    )

    with (
        patch("app.users.services.login.signin_with_username_password", return_value=mock_response) as sign_in,
        ...
        response = await signin_with_password(user_login_request_data, mock_client)
