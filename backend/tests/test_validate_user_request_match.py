...
class TestValidateUserRequestMatch:
    ...
    @pytest.mark.asyncio
    @patch(USER_INFO_PATH, new_callable=AsyncMock)
    async def test_valid_user_match_passes(self, mock_get_user_info):
        ...
        mock_get_user_info.assert_called_once_with(
            mock_request.app.state.request_client, "123"
        )
    ...
    async def test_invalid_profile_type_raises_400(self, mock_get_user_info):
        ...
        for invalid_data in invalid_profile_data_types:
            ...
            with pytest.raises(
                AttributeError
            ):  # Will raise AttributeError when trying to access .id on invalid types
                await validate_user_id_matches_session(
                    mock_request, "123", request_user_id
                )
        ...
        with pytest.raises(
            AttributeError
        ):  # Will raise AttributeError when trying to access .id on None
            await validate_user_id_matches_session(mock_request, "123", request_user_id)
