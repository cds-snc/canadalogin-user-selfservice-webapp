...
class TestSMSEnrollment:
    ...
    @pytest.mark.asyncio
    async def test_handle_sms_otp_enrollment_checks_and_records_rate_limit(
        self,
        mock_sms_enrollment_request,
        mock_user_profile_response,
        mock_ibm_enrollment_response,
        ...
        with patch("app.otp.services.enroll_mfa_otp.get_my_profile") as mock_my_profile:
            ...
            with patch(
                "app.otp.services.enroll_mfa_otp.get_user_otp_factors"
            ) as mock_get_user_otp_factors:
                mock_get_user_otp_factors.return_value = MagicMock(
                    success=True,
                    data=[MagicMock(type="smsotp")],
                )
    ...
    @pytest.mark.asyncio
    async def test_handle_sms_otp_enrollment_raises_when_rate_limited(
        self,
        mock_sms_enrollment_request,
        mock_user_profile_response,
    ):
    ...
    @pytest.mark.asyncio
    async def test_handle_sms_otp_enrollment_raises_when_phone_factor_capacity_reached(
        self,
        mock_sms_enrollment_request,
        mock_user_profile_response,
    ):
        ...
        with patch("app.otp.services.enroll_mfa_otp.get_my_profile") as mock_my_profile:
            ...
            with patch(
                ...
                mock_get_user_otp_factors.return_value = MagicMock(
                    success=True,
                    data=[
                        MagicMock(type="smsotp"),
                        MagicMock(type="voiceotp"),
                        MagicMock(type="smsotp"),
                        MagicMock(type="voiceotp"),
                    ],
                )
                ...
                with patch(
                    ...
                    assert exc.value.detail == "mfa_phone_max_factors"
                    mock_dispatch.assert_not_called()

    @pytest.mark.asyncio
    async def test_handle_sms_otp_enrollment_allows_when_phone_factor_capacity_below_limit(
        self,
        mock_sms_enrollment_request,
        mock_user_profile_response,
        mock_ibm_enrollment_response,
