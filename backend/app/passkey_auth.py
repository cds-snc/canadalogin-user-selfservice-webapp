...
class PasskeyAuth:
    ...
    def log_api_call(self, method: str, url: str, headers: dict, data: dict = None, params: dict = None, response: requests.Response = None):
        ...
        masked_headers = {k: v if k.lower() not in ['authorization'] else '***' for k, v in headers.items()}
    ...
    async def get_admin_token(self):
        ...
        try:
            ...
            self.log_api_call("POST", token_url, headers, data, response=response)
    ...
    async def get_user_id(self, username: str, admin_token: str) -> str:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Failed to get user. Status: {response.status_code}")
            ...
            raise HTTPException(status_code=400, detail=f"Error getting user ID: {str(e)}")
    ...
    async def generate_authentication_options(self, username: str):
        ...
        try:
            ...
            logger.info(f"Making assertion options request to: {auth_options_url}")
            logger.info(f"Request headers: {headers}")
            logger.info(f"Request data: {options_data}")
            
            response = requests.post(auth_options_url, json=options_data, headers=headers)
            logger.info(f"Response status: {response.status_code}")
            logger.info(f"Response: {response.text}")
            
            if response.status_code != 200:
                logger.error(f"Failed to generate assertion options. Status: {response.status_code}")
                logger.error(f"Error response: {response.text}")
                raise HTTPException(status_code=400, detail="Failed to generate authentication options")
                
            return response.json()

        except Exception as e:
            logger.error(f"Error generating assertion options: {str(e)}", exc_info=True)
            raise HTTPException(status_code=400, detail=f"Authentication options error: {str(e)}")
    ...
    async def verify_authentication(self, username: str, credential: dict):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Failed to verify assertion. Status: {response.status_code}")
                raise HTTPException(status_code=400, detail="Failed to verify authentication")
                
            verification_token = response.json().get("verification_token")
            if not verification_token:
                raise HTTPException(status_code=400, detail="No verification token received")
            ...
            raise HTTPException(status_code=400, detail=f"Authentication verification error: {str(e)}")
    ...
    async def get_user_token(self, token_data: dict):
        ...
        try:
            ...
            self.log_api_call("POST", token_url, headers, token_data, response=response)
            
            if response.status_code != 200:
                logger.error(f"Failed to get user token. Status: {response.status_code}")
                raise HTTPException(status_code=400, detail="Failed to get user token")
            ...
            raise HTTPException(status_code=400, detail=f"User token error: {str(e)}")
