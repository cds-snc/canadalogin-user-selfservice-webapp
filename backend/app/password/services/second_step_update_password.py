...
from app.password.schemas import SecondStepPasswordUpdatePayload, UpdatePasswordIbmApiResponse, UpdatePasswordClientResponsePayload
...
async def second_step_update_password(global_http_client: AsyncClient, payload: SecondStepPasswordUpdatePayload):
    ...
    try:
        ...
        password_otp_response = await dispatch_password_otp_validator(global_http_client, payload)
...
async def dispatch_password_otp_validator(global_http_client: AsyncClient, payload: SecondStepPasswordUpdatePayload):
    """The global_http_client is a httpx AsyncClient connection pool, created at startup time. It can be found in main.py
    Use it for ALL API calls."""
    ...
    try:
        ...
        password_resetter_otp_api_endpoint = f"{settings.password_resetter_api_endpoint}/{payload.trxId}/validator"

        form_data = {
            "otp": payload.otp,
        }

        logger.info(f"Form data for password reset: {password_resetter_otp_api_endpoint} => {form_data}")
        response = await global_http_client.post(
            password_resetter_otp_api_endpoint, json=form_data, headers=headers
        )
        logger.info(f"returned response from password_resetter_otp_api_endpoint: {response.json()}")
