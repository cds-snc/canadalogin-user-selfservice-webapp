...
async def handle_otp_send(user_otp_info: UserOtpInfo, global_http_client: AsyncClient, admin_token_cache_file):
    """The global_http_client is a httpx AsyncClient connection pool, created at startup time. It can be found in main.py
    Use it for ALL API calls."""
    print(admin_token_cache_file.name)
    try:
        logger.info(f"Attempting to send {user_otp_info.otpType} OTP")
        start_time = datetime.now()
        http_client_response = await dispatch_otp(user_otp_info, global_http_client, admin_token_cache_file)
        ...
        if http_client_response.status_code != 201:
            ...
            return generate_error_response(
                http_client_response.status_code, http_client_response.json()
            )
...
async def dispatch_otp(user_otp_info: UserOtpInfo, global_http_client: AsyncClient, admin_token_cache_file):
    """The global_http_client is a httpx AsyncClient connection pool, created at startup time. It can be found in main.py
    Use it for ALL API calls."""
