...
async def handle_otp_status_retrieval(retrieval_data: RetrievalData, global_http_client: AsyncClient):
    try:
        logger.info(f"Attempting to retrieve {retrieval_data.otpType} OTP.")
        start_time = datetime.now()
        http_client_response = await dispatch_otp_status_retrieval(retrieval_data, global_http_client)
...
async def dispatch_otp_status_retrieval(retrieval_data: RetrievalData, global_http_client: AsyncClient):
    try:
        ...
        logger.error(f"HTTP Exception in {retrieval_data.otpType} OTP status check: {str(he)}")
        raise he
