...
class MFASignup:
    ...
    def log_api_call(self, method: str, url: str, headers: dict, data: dict = None, params: dict = None, response: requests.Response = None):
        ...
        # Mask sensitive data
        masked_headers = {k: v if k.lower() not in ['authorization', 'password'] else '***' for k, v in headers.items()}
        masked_data = {k: v if k.lower() != 'password' else '***' for k, v in (data or {}).items()}
    ...
    async def create_user(self, user_data: dict):
        ...
        try:
            ...
            response = requests.post(users_url, json=verify_user_data, headers=headers)
            self.log_api_call("POST", users_url, headers, verify_user_data, response=response)
            
            if response.status_code != 201:
                logger.error(f"Failed to create user. Status: {response.status_code}")
            ...
            raise HTTPException(status_code=400, detail=f"User creation error: {str(e)}")
    ...
    async def generate_totp_qr(self, user_id: str):
        ...
        try:
            ...
            self.log_api_call("POST", totp_url, headers, totp_data, response=response)
            
            if response.status_code != 201:
                logger.error(f"Failed to generate TOTP QR. Status: {response.status_code}")
                logger.error(f"Response: {response.text}")
                raise HTTPException(status_code=400, detail="Failed to generate TOTP QR code")
            
            response_data = response.json()
            logger.info("TOTP response data: %s", response_data)
            logger.info("TOTP response attributes: %s", response_data.get('attributes', {}))
            ...
            logger.info("TOTP enrollment created successfully. Status: %s", totp_enrollment['status'])
            ...
            raise HTTPException(status_code=400, detail=f"TOTP QR generation error: {str(e)}")
    ...
    async def verify_totp(self, user_id: str, totp_id: str, code: str):
        ...
        try:
            ...
            self.log_api_call("POST", verify_url, headers, verify_data, response=response)
            
            if response.status_code not in [200, 204]:
                logger.error(f"Failed to verify TOTP. Status: {response.status_code}")
                logger.error(f"Response: {response.text}")
            ...
            raise HTTPException(status_code=400, detail=f"TOTP verification error: {str(e)}")
    ...
    async def signup_with_mfa(self, user_data: dict, totp_code: str = None):
        ...
        try:
            ...
            if totp_code:
                ...
                if not totp_id:
                    raise HTTPException(status_code=400, detail="Failed to get TOTP ID")
            ...
            raise HTTPException(status_code=400, detail=f"MFA signup error: {str(e)}")
