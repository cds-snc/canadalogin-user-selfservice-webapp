...
async def verify_user_exists(username: str, admin_token: str):
    ...
    try:
        ...
        response = requests.get(users_url, headers=headers, params=search_params)
        log_api_call("GET", users_url, headers, params=search_params, response=response)
        
        if response.status_code != 200:
            logger.error(f"Failed to verify user. Status: {response.status_code}")
...
async def authenticate_password(username: str, password: str):
    ...
    try:
        ...
        tenant_url = settings.IBM_VERIFY_TENANT_URL.rstrip('/')  # Remove trailing slash if present
        password_source_id = "3546b745-8790-4578-b4b5-401390cbc9b4"  # Password identity source identifier
        ...
        log_api_call("POST", initiate_url, initiate_headers, initiate_data, response=initiate_response)
        
        if initiate_response.status_code != 200:
            logger.error(f"Authentication failed. Status: {initiate_response.status_code}")
        ...
        log_api_call("POST", token_url, token_headers, token_data, response=token_response)
        
        if token_response.status_code != 200:
            logger.error(f"Token request failed. Status: {token_response.status_code}")
        ...
        result = {
            ...
            **initiate_response.json()  # Include all fields from the password authentication response
        }
...
class PasswordAuth:
    ...
    async def verify_password(self, username: str, password: str):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Password verification failed. Status: {response.status_code}")
                raise HTTPException(status_code=401, detail="Invalid credentials")
